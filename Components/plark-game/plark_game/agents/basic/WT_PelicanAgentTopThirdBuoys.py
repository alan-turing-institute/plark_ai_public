...
class PelicanAgentTopThirdBuoys(PelicanAgentFixedSBs):

    def __init__(self):
        super(PelicanAgentTopThirdBuoys, self).__init__()

    def getAction(self, state):
        if len(self.sb_locations) == 0:
            self.sb_locations = self.generate_sb_locations(state)

        return super(PelicanAgentTopThirdBuoys, self).getAction(state)
    ...
    def generate_sb_locations(self, state):
        ...
        # Start 1/3 of the way down the grid, floor value
        sb_locations.append({'col': sb_range, 'row': num_rows//3 - 1})

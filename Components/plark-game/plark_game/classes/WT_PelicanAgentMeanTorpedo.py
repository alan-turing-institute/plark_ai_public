...
class PelicanAgentMeanTorpedo(Pelican_Agent):
    """
    ...
    def getAction(self, state):
        ...
        # Find only HOT sbs
        hot_sbs = [
            {'col': cell['coordinate'][0], 'row': cell['coordinate'][1]} for cell
            in deployed_sb_cells if cell['objects']['SONOBUOY'] == 'HOT']

        if len(hot_sbs) == 0:
            return self.action_lookup(8)  # end

        elif len(hot_sbs) == 1: # If only one hot sb then use previous method
            # hot_sbs[0]['col']
            if game_helper.distance(pelican_col, pelican_row, hot_sbs[0]['col'], hot_sbs[0]['row']) > state['sb_range']:
                path = game_helper.get_path(pelican_col, pelican_row, hot_sbs[0]['col'], hot_sbs[0]['row'], num_cols, num_rows)
            ...
            avg_col = sum([sb["col"] for sb in hot_sbs]) // 2
            avg_row = sum([sb["row"] for sb in hot_sbs]) // 2

            if (pelican_col != avg_col) or (pelican_row != avg_row): # drop on this exact cell
                path = game_helper.get_path(pelican_col, pelican_row, avg_col, avg_row, num_cols, num_rows)
                return self.action_lookup(self.cell_to_action(path[0], pelican_col, pelican_row))
            # If pelican is on cell, drop torpedo if there isn't one nearby already
            else:
